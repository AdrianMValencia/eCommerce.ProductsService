...
) : IQueryHandler<GetProductByIdQuery, GetProductByIdResponseDto>
{
    ...
    public async Task<BaseResponse<GetProductByIdResponseDto>> Handle(GetProductByIdQuery query, CancellationToken cancellationToken)
    {
        var response = new BaseResponse<GetProductByIdResponseDto>();
        try
        {
            var product = await _unitOfWork.ProductRepository.GetProductByIdAsync(query.ProductId, cancellationToken);
            if (product == null)
            {
